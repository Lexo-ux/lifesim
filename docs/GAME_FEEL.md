...
**Task 06 integration:** the live Awakening slice now calls these same semantic states through a read-only adapter. No additional FX owner or renderer exists. The laboratory retains its visual-only fixture and adds explicit isolated in-memory life cases using known input seeds and the production engine; it never imports persistence or touches normal saves. See [AWAKENING_SYSTEM](AWAKENING_SYSTEM.md) for the new callers and transaction boundary.

`tools/game-feel-lab.html` is an unlinked, noindex public developer tool, not a secret/admin feature. Its original visual-only mode uses an isolated checked-in V3 fixture. Task 06 additionally imports the engine for explicitly selected in-memory examples; no app/storage module is imported and no normal save is read or written. It exposes all ten visual states, quality selection, cancellation and a timed normal → Convergence → S → SS → SSS sequence. In visual-only mode, decisions rehearse card contact/exit without advancing a life; selected Task 06 examples advance only their isolated fixture. Its explicit QA handle exists only there. No development module is imported by production.
