...
HTML carga `style.css` y `src/main.js` con rutas relativas al documento. CSS importa fuentes locales y módulos estáticos. `src/ui/art.js` mapea dos pilotos de arte; NPCs y entornos restantes conservan su resolución legacy. Task 05 centraliza los doce retratos del protagonista en `src/ui/characters.js`; `creator.js` presenta apariencia/edad de vista previa sin tocar estado ni PRNG. `helpers.playerPortrait` aplica la etapa real al mismo registro para Moments, perfil y memorial. La UI construye URLs WebP por NPC/apariencia/etapa; `street` corresponde a `neighborhood.webp`. Precarga el siguiente retrato al elegir. `motion.js` posee efectos y limpieza; `threshold.js` posee los estados, efectos ambientales, un timer y listeners del título, sin acceso al motor/PRNG; `swipe.js` desmonta listeners con AbortController. Nada visual consume el PRNG. Ver [assets](ASSETS.md).
