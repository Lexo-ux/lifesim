...
/* Cada lugar trae su país y moneda real. El stat de dinero sigue
   siendo 0-100 por dentro (para que el balance del juego sea el
   mismo en todos lados), pero lo que se muestra en pantalla se
   convierte a un monto con la moneda de ese país. "factor" es
   cuánto vale cada punto de dinero en esa moneda — son cifras
   pensadas para sonar creíbles, no una conversión económica real.*/
const LUGARES = [
  { ciudad:"Bogotá", pais:"Colombia", moneda:"COP", simbolo:"$", factor:40000 },
  { ciudad:"Medellín", pais:"Colombia", moneda:"COP", simbolo:"$", factor:40000 },
  { ciudad:"Cali", pais:"Colombia", moneda:"COP", simbolo:"$", factor:40000 },
  { ciudad:"Barranquilla", pais:"Colombia", moneda:"COP", simbolo:"$", factor:40000 },
  { ciudad:"Ciudad de México", pais:"México", moneda:"MXN", simbolo:"$", factor:2000 },
  { ciudad:"Guadalajara", pais:"México", moneda:"MXN", simbolo:"$", factor:2000 },
  { ciudad:"Buenos Aires", pais:"Argentina", moneda:"ARS", simbolo:"$", factor:15000 },
  { ciudad:"Lima", pais:"Perú", moneda:"PEN", simbolo:"S/", factor:40 },
  { ciudad:"Santiago de Chile", pais:"Chile", moneda:"CLP", simbolo:"$", factor:9000 },
  { ciudad:"Quito", pais:"Ecuador", moneda:"USD", simbolo:"$", factor:20 },
  { ciudad:"Montevideo", pais:"Uruguay", moneda:"UYU", simbolo:"$", factor:700 },
  { ciudad:"San José", pais:"Costa Rica", moneda:"CRC", simbolo:"₡", factor:10000 }
];

function formatearDinero(jugador){
  const monto = Math.round((jugador.dinero * jugador.monedaFactor) / 1000) * 1000;
  return `${jugador.monedaSimbolo}${monto.toLocaleString("es-CO")} ${jugador.monedaCodigo}`;
}
...
/* Config visual del muñeco de la senda de vida: escala (tamaño),
   color (cicla mostaza/coral/teal como el resto de la marca) y si
   lleva bastón en la vejez. */
const VISUAL_ETAPA = {
  bebe: { escala:0.55, color:"#F2A93B" },
  ninez: { escala:0.68, color:"#F0563D" },
  adolescencia: { escala:0.8, color:"#2BB3A3" },
  joven_adulto: { escala:0.92, color:"#F2A93B" },
  adultez: { escala:1, color:"#F0563D" },
  vejez: { escala:0.88, color:"#2BB3A3", baston:true }
};
const EDAD_MAXIMA_SENDA = 90;

const EVENTOS = {
  bebe: [
    { tipo:"narrativa", variantes:["Naciste llorando a todo pulmón. El doctor dice que tienes buenos pulmones para el karaoke.", "Naciste en completo silencio, mirando a todos con cara de estar juzgando la decoración del hospital."], deltas:{felicidad:5} },
    ...
    { tipo:"narrativa", variantes:["Diste tus primeros pasos, directo hacia el perro de la casa. El perro no se movió. Tú sí, para atrás.", "Diste tus primeros pasos directo hacia la mesa de centro. La mesa ganó esa ronda."], deltas:{salud:-2, felicidad:5} },
  ...
  ninez: [
    ...
    { tipo:"decision", prompt:"Un compañero te reta a comerte una lombriz de tierra por 2.000 pesos.", opciones:[
      { texto:"Aceptar el reto", deltas:{dinero:5, felicidad:8, salud:-4}, resultado:"Te la comiste. Ahora eres leyenda del salón (y tienes náuseas).", tag:"comio_lombriz" },
      { texto:"Rechazar con dignidad", deltas:{inteligencia:2, felicidad:1}, resultado:"Te fuiste caminando lento, como en las películas." }
    ]},
    { tipo:"narrativa", variantes:["Te caíste de la bicicleta intentando un truco que viste en internet. Al menos quedó en video.", "Intentaste enseñarle un truco de bicicleta a un amigo y terminaste demostrando exactamente cómo NO hacerlo."], deltas:{salud:-5, felicidad:3} },
    ...
    { tipo:"narrativa", variantes:["Descubriste los videojuegos. Tus notas empezaron una caída libre.", "Un primo te prestó su consola «solo por un rato». Ese rato duró todo el fin de semana."], deltas:{felicidad:6, inteligencia:-3} },
    { tipo:"decision", prompt:"Tus papás te preguntan qué quieres ser cuando grande.", opciones:[
      ...
      { texto:"Youtuber", deltas:{felicidad:5, dinero:-1}, resultado:"Grabaste tu primer video. Nadie lo vio, pero tú lo disfrutaste.", tag:"sonaba_con_youtube" }
    ]},
  ...
  adolescencia: [
    { tipo:"narrativa", variantes:["Te dio el estirón. Ya no reconoces tu propia voz cuando hablas.", "Creciste tanto en pocos meses que toda tu ropa de repente parecía de otra persona."], deltas:{salud:2, felicidad:-2} },
    ...
    { tipo:"decision", prompt:"Te invitan a una fiesta la misma noche que tienes examen.", opciones:[
      { texto:"Ir a la fiesta", deltas:{felicidad:10, inteligencia:-5}, resultado:"Bailaste toda la noche. El examen fue... una experiencia.", tag:"prioriza_diversion" },
      { texto:"Quedarte a estudiar", deltas:{inteligencia:6, felicidad:-4}, resultado:"Sacaste buena nota. Tus amigos te mandaron fotos de la fiesta.", tag:"prioriza_estudio" }
    ]},
    { tipo:"narrativa", variantes:["Conseguiste tu primer trabajo de medio tiempo repartiendo volantes. Ganaste menos de lo que esperabas, pero te sentiste adulto.", "Conseguiste tu primer trabajo de medio tiempo en una heladería. Te prohibieron comer del inventario después del tercer día."], deltas:{dinero:4, felicidad:2, salud:-1} },
    ...
    { tipo:"decision", prompt:"Un amigo te invita a hacer una locura para un reto viral.", opciones:[
      { texto:"Hacerlo", deltas:{felicidad:8, salud:(j)=> j.salud < 40 ? -10 : -6}, resultado:"Sobreviviste. El video tuvo más vistas que tu grupo familiar de WhatsApp.", tag:"hizo_reto_viral" },
  ...
  joven_adulto: [
    { tipo:"decision", prompt:"Terminaste el colegio. ¿Qué haces con tu vida?", opciones:[
      { texto:"Ir a la universidad", deltas:{inteligencia:8, dinero:-5}, resultado:"Empezaste una carrera. El café ahora es tu combustible principal.", tag:"fue_universidad" },
      { texto:"Buscar trabajo directo", deltas:{dinero:8, inteligencia:-2}, resultado:"Conseguiste tu primer sueldo. Te sentiste millonario por dos días.", tag:"trabajo_directo" }
    ]},
    { tipo:"narrativa", variantes:["Te mudaste solo/a por primera vez. Descubriste que la comida no se cocina sola.", "Te mudaste solo/a por primera vez. La primera semana comiste cereal en las tres comidas del día."], deltas:{felicidad:4, dinero:-4, inteligencia:2} },
    ...
    { tipo:"decision", prompt:"Te ofrecen una tarjeta de crédito con un límite tentador.", opciones:[
      { texto:"Sacarla y gastar sin culpa", deltas:{felicidad:6, dinero:(j)=> j.dinero < 40 ? -14 : -10}, resultado:"Ese viaje a la playa valió cada peso... que no tenías." },
    ...
    { tipo:"decision", prompt:"Un amigo te propone emprender un negocio juntos.", opciones:[
      { texto:"Meterte de lleno", deltas:{dinero:2, felicidad:6, inteligencia:3}, resultado:"El negocio va lento, pero cada venta se siente como ganar la lotería.", tag:"emprendio_joven" },
    ...
    { tipo:"narrativa", variantes:["Adoptaste una mascota. Ahora tu vida gira alrededor de otro ser vivo con más energía que tú.", "Adoptaste una mascota que se suponía era «de bajo mantenimiento». Mentira absoluta, pero no te arrepientes."], deltas:{felicidad:7, dinero:-3} },
  ...
  adultez: [
    ...
    { tipo:"decision", prompt:"Te ofrecen invertir tus ahorros en un negocio «seguro» que te recomendó un conocido.", opciones:[
      { texto:"Invertir todo", deltas:{dinero:(j)=> -Math.round(j.dinero * 0.5), felicidad:-5}, resultado:"El «negocio seguro» resultó ser menos seguro de lo prometido." },
    ...
    { tipo:"narrativa", variantes:["Empezaste a hacer ejercicio «en serio». Tu espalda ahora truena como palomitas de maíz.", "Te inscribiste en un gimnasio motivado/a por un video en redes. Fuiste 4 veces esa primera semana y luego... bueno, ya sabes."], deltas:{salud:8, felicidad:2} },
    ...
    { tipo:"narrativa", variantes:["Compraste una planta para «darle vida» a tu casa. Le pusiste nombre. Sigue viva, milagrosamente.", "Alguien te regaló una planta «imposible de matar». Sigues comprobando si es verdad, semana tras semana."], deltas:{felicidad:4} },
    ...
    { tipo:"decision", prompt:"Se abre la oportunidad de cambiar de carrera por completo.", opciones:[
      { texto:"Dar el salto", deltas:{dinero:-6, felicidad:9, inteligencia:4}, resultado:"Empezar de cero da miedo, pero por fin sientes que haces algo tuyo.", tag:"cambio_carrera" },
  ...
  vejez: [
    ...
    { tipo:"narrativa", variantes:["Empezaste a contar la misma historia por tercera vez esta semana. Tu familia finge no haberla escuchado antes.", "Le diste el mismo consejo a tu nieto que le diste la semana pasada. Él fingió que era la primera vez que lo escuchaba."], deltas:{felicidad:3} },
    ...
    { tipo:"narrativa", variantes:["Te compraste unos audífonos nuevos porque «ya nadie habla claro». El problema no eran ellos.", "Empezaste a decir «en mis tiempos» por lo menos tres veces al día. Te encanta el nuevo poder que sientes."], deltas:{salud:2, felicidad:1} },
...
/* Eventos «callback»: solo aparecen si antes elegiste la opción con el
   tag que piden, y solo una vez por partida. Esto es lo que le da
   memoria al juego sin necesitar ninguna IA en vivo. */
const EVENTOS_CALLBACK = [
  { requiere:"sonaba_con_youtube", etapaId:"joven_adulto", evento:
    { tipo:"narrativa", texto:"Te acordaste de que de niño querías ser youtuber. Grabaste un video random a las 2am y por alguna razón se volvió viral.", deltas:{felicidad:8, dinero:5} } },
  { requiere:"comio_lombriz", etapaId:"adolescencia", evento:
    { tipo:"narrativa", texto:"En una fiesta, alguien te reta de nuevo a comer algo raro. Ya tienes experiencia de sobra.", deltas:{felicidad:5, salud:-2} } },
  { requiere:"prioriza_diversion", etapaId:"joven_adulto", evento:
    { tipo:"decision", prompt:"Te invitan a otra fiesta la noche antes de una entrevista de trabajo importante.", opciones:[
      { texto:"Ir de todas formas, ya es tu estilo", deltas:{felicidad:8, dinero:-3}, resultado:"La entrevista fue... memorable, por las razones equivocadas." },
      { texto:"Por primera vez, quedarte a descansar", deltas:{dinero:6, inteligencia:2}, resultado:"Sorprendiste hasta a ti mismo/a. La entrevista salió perfecta." }
    ]} },
  { requiere:"prioriza_estudio", etapaId:"adultez", evento:
    { tipo:"narrativa", texto:"Todo ese esfuerzo de estudiar en vez de salir de fiesta en el colegio por fin rindió frutos: te ofrecieron dirigir un proyecto importante.", deltas:{dinero:8, inteligencia:3} } },
  { requiere:"hizo_reto_viral", etapaId:"joven_adulto", evento:
    { tipo:"narrativa", texto:"Ese video viral de la adolescencia resurgió en redes. Ahora la gente te reconoce en la calle por eso, para bien o para mal.", deltas:{felicidad:4} } },
  { requiere:"fue_universidad", etapaId:"adultez", evento:
    { tipo:"narrativa", texto:"El título universitario por fin se sintió útil: te lo pidieron justo para un ascenso que casi nadie más calificaba.", deltas:{dinero:6, inteligencia:2} } },
  { requiere:"trabajo_directo", etapaId:"adultez", evento:
    { tipo:"narrativa", texto:"Todos esos años de experiencia trabajando en vez de estudiar te pusieron por delante de colegas con más diplomas y menos calle.", deltas:{dinero:7} } },
  { requiere:"emprendio_joven", etapaId:"adultez", evento:
    { tipo:"decision", prompt:"Ese negocio que empezaste con tu amigo hace años por fin te ofrece un giro grande.", opciones:[
      { texto:"Venderlo todo y asegurar la ganancia", deltas:{dinero:15}, resultado:"Efectivo en mano. Duermes tranquilo/a esta noche." },
      { texto:"Seguir apostando en grande", deltas:{dinero:-5, felicidad:8}, resultado:"Riesgoso, pero se siente como estar viviendo la película de tu vida." }
    ]} },
  { requiere:"cambio_carrera", etapaId:"vejez", evento:
    { tipo:"narrativa", texto:"Mirando atrás, ese salto de carrera que diste fue de las mejores decisiones que tomaste. Nunca te arrepentiste.", deltas:{felicidad:6} } }
];
...
const diario = $("diario");
const sendaPersonaje = $("senda-personaje");
const sendaEscala = $("senda-escala");
const sendaCabeza = $("senda-cabeza");
const sendaCuerpo = $("senda-cuerpo");
const sendaPierna1 = $("senda-pierna1");
const sendaPierna2 = $("senda-pierna2");
const sendaBaston = $("senda-baston");
const inputNombre = $("input-nombre");
const botonesSexo = document.querySelectorAll(".sexo-boton");
...
function siguienteEvento(etapaId){
  const callback = EVENTOS_CALLBACK.find(c =>
    c.etapaId === etapaId &&
    jugador.historial.includes(c.requiere) &&
    !jugador.callbacksUsados.has(c.requiere)
  );
  if(callback){
    jugador.callbacksUsados.add(callback.requiere);
...
function valorDelta(d){ return typeof d === "function" ? d(jugador) : d; }

function aplicarDeltas(deltas){
  if(!deltas) return;
  if(deltas.salud) jugador.salud = clamp(jugador.salud + valorDelta(deltas.salud));
  if(deltas.felicidad) jugador.felicidad = clamp(jugador.felicidad + valorDelta(deltas.felicidad));
  if(deltas.inteligencia) jugador.inteligencia = clamp(jugador.inteligencia + valorDelta(deltas.inteligencia));
  if(deltas.dinero) jugador.dinero = clamp(jugador.dinero + valorDelta(deltas.dinero));
}

function actualizarSendaVida(){
  const etapa = etapaDeEdad(jugador.edad);
  const visual = VISUAL_ETAPA[etapa.id];
  const porcentaje = Math.min(jugador.edad / EDAD_MAXIMA_SENDA, 1) * 100;

  sendaPersonaje.style.left = porcentaje + "%";
  sendaEscala.style.transform = `scale(${visual.escala})`;
  sendaCabeza.style.fill = visual.color;
  sendaCuerpo.style.fill = visual.color;
  sendaPierna1.style.stroke = visual.color;
  sendaPierna2.style.stroke = visual.color;
  sendaBaston.style.display = visual.baston ? "block" : "none";
  sendaBaston.style.stroke = visual.color;

  sendaEscala.classList.remove("caminando");
  void sendaEscala.offsetWidth;
  sendaEscala.classList.add("caminando");
}

function actualizarUI(){
  $("nombre-jugador").textContent = `${jugador.nombre} · ${jugador.ciudad}`;
  const etapa = etapaDeEdad(jugador.edad);
  $("avatar").className = `avatar-sprite etapa-${etapa.id} pelo-${jugador.pelo}`;
  ...
  ["salud","felicidad","inteligencia","dinero"].forEach(stat => {
    $(`valor-${stat}`).textContent = stat === "dinero" ? formatearDinero(jugador) : jugador[stat];
...
/* -------- ciclo de vida -------- */
function iniciarJuego(aleatorioForzado){
  const nombreForm = inputNombre.value.trim();
  const nombre = (aleatorioForzado || !nombreForm) ? elegir(NOMBRES) : nombreForm;

  let pelo;
  const sexo = aleatorioForzado ? null : sexoElegido;
  if(sexo === "hombre") pelo = "corto";
  else if(sexo === "mujer") pelo = "largo";
  else pelo = Math.random() < 0.5 ? "corto" : "largo";
  ...
  jugador = {
    ...
    ciudad: lugar.ciudad,
    pais: lugar.pais,
    monedaCodigo: lugar.moneda,
    monedaSimbolo: lugar.simbolo,
    monedaFactor: lugar.factor,
    edad: 0,
...
function resolverEvento(evento){
  if(evento.tipo === "narrativa"){
    ...
    const texto = evento.variantes ? elegir(evento.variantes) : evento.texto;
    agregarEntradaDiario(jugador.edad, texto);
    actualizarUI();
...
function mostrarModalDecision(evento){
  ...
  evento.opciones.forEach(opcion => {
    ...
    boton.addEventListener("click", () => {
      ...
      if(opcion.tag) jugador.historial.push(opcion.tag);
...
function terminarJuego(mensajeMuerte){
  ...
  $("final-resumen").textContent = `${mensajeMuerte} Viviste ${jugador.edad} años como ${jugador.nombre}, en ${jugador.ciudad}, ${jugador.pais}.`;
  ...
  $("final-stats").innerHTML = `
    ...
    <span class="final-stat-pill">💰 ${formatearDinero(jugador)}</span>
  `;
...
botonesSexo.forEach(boton => {
  boton.addEventListener("click", () => {
    botonesSexo.forEach(b => b.classList.remove("activo"));
    boton.classList.add("activo");
    sexoElegido = boton.dataset.sexo;
  });
});

btnNacer.addEventListener("click", () => iniciarJuego(false));
btnAleatorio.addEventListener("click", () => iniciarJuego(true));
btnEnvejecer.addEventListener("click", envejecer);
btnReiniciar.addEventListener("click", () => mostrarPantalla("pantalla-inicio"));
