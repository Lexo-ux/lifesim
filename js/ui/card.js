...
export function portraitFor(s, id) {
  ...
  return `assets/npcs/${id}${ages[id] ? "-" + ages[id] : ""}.webp`;
}
export function gameScreen(data) {
 ...
 <div class="portrait-window" style="--scene:url('assets/backgrounds/${environment}.webp')"><span class="card-corner" aria-hidden="true">${icon(e.pool === "meta" ? "moon" : "spark")}</span><img class="npc-portrait ${e.npc === "self" ? "self-portrait" : ""}" src="${portraitFor(s, e.npc)}" alt="${esc(npc.name)}" draggable="false" fetchpriority="high"><div class="speaker"><p>${esc(npc.role)}</p><h1 id="page-title">${esc(npc.name)}</h1></div></div>
 <div class="dialogue"><p id="card-dialogue">${esc(cardText(s, data.meta))}</p></div><div class="choice-preview preview-left" aria-hidden="true">${esc(e.left.label)}</div><div class="choice-preview preview-right" aria-hidden="true">${esc(e.right.label)}</div></article></div>
