...
function pathAvailable(s,event) {
  ...
  if(event.test?.startsWith('job:'))return s.career?.id!==event.test.slice(4);
  if(event.test?.startsWith('course:')) {
    const course=COURSES.find(c=>c.id===event.test.slice(7));
    return !s.education.degrees.includes(course.id)&&s.cash>=Math.round(course.cost*(s.flags.scholarship?0.5:1));
  }
  return true;
}
export function weight(s,event) {
  let w=event.weight??({common:5,uncommon:2,rare:.6,legendary:.15,secret:1}[event.rarity]||1);
  ...
  if(s.story.recent.slice(-2).some(id=>CARD_BY_ID[id]?.npc===event.npc))w*=.3;
  if(s.story.recent.includes(event.id))w*=.1;
  return w;
}
export function drawCard(s,meta) {
  if(!s.alive){s.story.current=null;return null;}
  const due=s.story.queue.find(q=>q.due<=now(s)&&eligible(s,meta,CARD_BY_ID[q.id],{queued:true}));
  let event=due?CARD_BY_ID[due.id]:null;
  if(due)s.story.queue.splice(s.story.queue.indexOf(due),1);
  if(!event) {
    const pool=CARDS.filter(e=>eligible(s,meta,e)&&pathAvailable(s,e));
    const priority=pool.filter(e=>e.priority).sort((a,b)=>b.priority-a.priority);
    if(priority.length)event=priority[0];
    else {
      let roll=random(s)*pool.reduce((sum,e)=>sum+weight(s,e),0);
      event=pool.find(e=>(roll-=weight(s,e))<0)||CARD_BY_ID.quiet_day;
    }
  }
...
export function cardText(s,meta,event=currentCard(s)) {
  const echo=meta.echoes.findLast(e=>e.id!==s.id)?.name||'un nombre que te resulta familiar';
  return event.text.replaceAll('{echo}',echo).replaceAll('{oldChoice}',meta.flags.envelopeKept?'guardarlo':'dejarlo');
}
