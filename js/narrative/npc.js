...
export function meet(s, id) {
  ...
  if (!npc) {
    npc = s.story.npcs[id] = { id, name:spec.name, role:spec.role, portrait:spec.portrait, firstMetAge:s.age, memories:[], flags:{}, alive:true, traits:[], bond:id==='elena'||id==='tomas'?78:50 };
    const existing=s.relationships.find(r=>r.id===id);
    if (spec.type && !existing && id!=='luz') s.relationships.push({id,name:spec.name,type:spec.type,bond:npc.bond,since:s.age});
  }
  return npc;
}
export function bond(s,id) { return s.relationships.find(r=>r.id===id)?.bond ?? s.story.npcs[id]?.bond ?? 50; }
...
export function npcYear(s) {
  for(const [id,npc] of Object.entries(s.story.npcs)) {
    ...
    if(npc.alive && s.age+spec.offset>=spec.lifespan) {
      npc.alive=false;
      log(s,`Te despediste de ${npc.name}. Conservaste sus recuerdos.`,true,'leaf');
      const relation=s.relationships.find(r=>r.id===id);
...
export function speaker(s,id) {
  ...
  const relation=s.relationships.find(r=>r.id===id);
  const role=relation?.type==='partner'?'Tu pareja':relation?.type==='ex'?'Un amor pasado':id==='luz'&&s.age-(relation?.since??s.age)>=18?'Tu hija adulta':npc.role;
  return {...npc,role};
}
