...
Inspect status and recent history first; preserve other work. Use the branch explicitly requested by the task (Task 07: `codex/life-paths-opportunities`), otherwise a `codex/` feature branch. Make descriptive commits. Never merge into `main` unless the user explicitly authorizes that action. Stop at the task boundary: Task 07 does not start Task 08. Read docs/AWAKENING_SYSTEM.md before changing Awakening, Core, class selection or its saves. Read docs/LIFE_PATHS.md before changing Life State, opportunities, capability projection or narrative memory; reuse the existing deck, queue and education/career authorities. Keep required closures reachable and external-context requirements fail-closed until their owner registers them. Read docs/ART_DIRECTION.md, docs/DESIGN_SYSTEM.md and docs/MOTION.md before changing presentation. Read docs/GAME_FEEL.md before changing gameplay presentation or FX; keep semantic effects independent of mechanics, RNG and saves. Read docs/CHARACTER_ART.md for the validated Veiled Identity protagonist families. Original protagonist sprites remain archival; NPC families remain legacy/pilot until separately validated.
