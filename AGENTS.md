...
Inspect status and recent history first; preserve other work. Use the branch explicitly requested by the task (Task 05: `codex/character-life-generation`), otherwise a `codex/` feature branch. Make descriptive commits. Never merge into `main` unless the user explicitly authorizes that action. Stop at the task boundary: Task 05 does not start Task 06 or implement future gameplay. Read docs/ART_DIRECTION.md, docs/DESIGN_SYSTEM.md and docs/MOTION.md before changing presentation. Read docs/CHARACTER_ART.md for the validated Veiled Identity protagonist families. Original protagonist sprites remain archival; NPC families remain legacy/pilot until separately validated.
